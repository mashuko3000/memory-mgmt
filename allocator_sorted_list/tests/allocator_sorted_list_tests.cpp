...
int main()
{
    // 1. Создаём аллокатор с 512 байт памяти
    allocator_sorted_list allocator(512);

    // 2. Аллоцируем место под строку "Hello, World!"
    const char* text = "Hello, World!";
    size_t len = strlen(text) + 1; // +1 для '\0'

    char* memory = static_cast<char*>(allocator.allocate(sizeof(char), len));
    ...
    // 3. Копируем строку в выделенную память
    strcpy(memory, text);

    // 4. Выводим строку из выделенной памяти
    std::cout << "Allocated string: " << memory << std::endl;
